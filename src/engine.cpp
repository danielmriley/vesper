...
void Engine::forward_token(int token) {
    ...
    embed_row(device_, x, weights_.tok_emb.data(), token, h);
    ...
    for (int layer_i = 0; layer_i < cfg.n_layers; ++layer_i) {
        ...
        rope_neox(device_, scratch_.q.data(), scratch_.k.data(), cfg.n_heads, cfg.n_kv_heads,
        ...
        for (int qh = 0; qh < cfg.n_heads; ++qh) {
            ...
            attn_mix(device_, scratch_.attn.data() + qh * cfg.head_dim, scratch_.scores.data(),
                     cache_.v[static_cast<std::size_t>(layer_i)].data(), seq, cfg.n_kv_heads,
                     kvh, cfg.head_dim);
        }
